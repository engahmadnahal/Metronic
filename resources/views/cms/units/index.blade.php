...
@section('page-name',__('cms.units'))
@section('main-page',__('cms.shop_content_management'))
@section('sub-page',__('cms.units'))
...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">{{__('cms.units')}}</span>
        ...
        <div class="card-toolbar">
            <a href="{{route('units.create')}}"
                ...
                <a href="{{route('units.exportExcel')}}"
            ...
            <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_2">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$unit->translations->first()?->name
                                ?? ''}}</a>
                        </td>
                        <td>
                            <a href="#" class="label label-lg label-light-success label-inline">{{$unit->code
                                ?? ''}}</a>
                        </td>
                        <td>
                            <a href="#" data-toggle="modal" data-target="#unit_{{$unit->id}}_translations"
                                class="btn btn-light-primary font-weight-bolder font-size-sm">({{$unit->translations_count}})</a>
                            <div class="modal fade" id="unit_{{$unit->id}}_translations" tabindex="-1" role="dialog"
                                aria-labelledby="unit_{{$unit->id}}_translations" aria-hidden="true">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="pr-0 text-right">
                                                            @can('Update-Country')
                                                            <a href="{{route('unit-translations.edit',$translation->id)}}"
                        ...
                        <td>
                            <span
                                class="label label-lg @if($unit->active) label-light-success @else label-light-warning @endif label-inline">{{$unit->active_key}}</span>
                        </td>
                        <td class="pr-0 text-right">
                            @can('Create-Country')
                            <a href="{{route('unit-translations.create',$unit->id)}}"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                            ...
                            @endcan
                            @can('Delete-Country')
                            <a href="#" onclick="performCountryDestroy('{{$unit->id}}', this)"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                                <span class="svg-icon svg-icon-md svg-icon-primary">
                                    <!--begin::Svg Icon | path:assets/media/svg/icons/General/Trash.svg-->
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
...
<script>
    function performCountryDestroy(id,reference) {
        confirmDestroy('/cms/admin/units', id, reference);
    }
    function performTranslationDestroy(id,reference) {
        confirmDestroy('/cms/admin/units/translations', id, reference);
    }
</script>
@endsection